...
- **Playback:** the service has its **own** `MediaPlayer` for **local** (Auto-initiated)
  playback + `MediaSessionCompat` + `AudioManager` focus + a MediaStyle notification.
  A **play-token** guards every `MediaPlayer` (and listeners are nulled before
  release) so stale `onCompletion`/`onError` callbacks from a released player can't
  advance/stop the next track — this fixes the "second skip stops playback" bug.
- **Voice:** `onPlayFromSearch(query)` → `findMatch` (mirrors the JS
  `matchTrackIndex`) → play the match, else **fall back to another song**.
  `onPlayFromMediaId` parses `listId|trackId`.
- **Phone ↔ car sync (single Auto-facing session):** the carmedia `MediaSession`
  is the **only** session Android Auto sees. It runs in one of two modes:
  - *Local* — Auto played something from the browse tree; carmedia's `MediaPlayer`
    is the audio, and `CarPlaybackBus.emitState` drives the in-app **`CarBanner`**.
  - *Proxy* — the phone (RNTP) is the audio; `playerStore.syncCarNowPlaying` →
    `CarMedia.setNowPlaying` mirrors the phone's metadata/state into the session so
    **Auto shows exactly what the phone plays**, and Auto's transport/voice commands
    are forwarded back via the `onCarCommand` event → `handleCarCommand` drives RNTP.
    `carSource.active` (shared flag) decides who owns the session so they never fight.
- **Custom actions** on the now-playing screen: **Like / Loop / Smart Radio**
  (`onCustomAction` → `onCarCommand` → toggle like / cycle repeat / start radio).
- **Browse styling:** category icons (Made for You/Most Played/Recently Played/
  Playlists/Songs) + per-item gradient "music-skeleton" artwork (`res/drawable`
  generated by `tools/gen-car-drawables.js`), set via `iconUri`; grid content style.
...
| Auto playback and phone playback fight / out of sync | One Auto-facing session (carmedia): *local* mode = carmedia's `MediaPlayer`; *proxy* mode = mirrors RNTP via `setNowPlaying`. `carSource.active` decides ownership. If they desync, check `syncCarNowPlaying` is firing (PlaybackState/ActiveTrackChanged/seek) and `onCarCommand` is wired in `attachListeners`. |
| Skipping in the car stops after one track | `playAt` uses a `playToken` and nulls a player's listeners before `release()`; without it, a released player's `onCompletion`/`onError` fires `onTrackComplete` and derails the next track. |
| Like/Loop/Smart-Radio buttons missing in Auto | They're `PlaybackStateCompat` custom actions added in `pushState()` with `R.drawable.ic_pb_*` icons; `onCustomAction` → `onCarCommand`. |
| Auto tiles look plain (no art) | Browse items set `iconUri` to `android.resource://…/drawable/ic_art_N` (+ category icons). If a head unit won't render vector resource URIs, the art falls back silently. |
